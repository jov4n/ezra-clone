...
func NewMusicExecutor(session *discordgo.Session, logger *zap.Logger, openRouterAPIKey string) *MusicExecutor {
	// Check dependencies
	if err := music.CheckDependencies(); err != nil {
		logger.Warn("Music dependencies not found", zap.Error(err))
	}

	// Set OpenRouter API key for playlist generation
	if openRouterAPIKey != "" {
		music.SetOpenRouterAPIKey(openRouterAPIKey)
		logger.Info("OpenRouter API key set for music playlist generation")
	} else {
		logger.Warn("OpenRouter API key not set - playlist generation will be limited")
	}

	return &MusicExecutor{
		manager: music.NewMusicManager(),
		session: session,
		logger:  logger,
	}
}
...
func (m *MusicExecutor) handlePlay(ctx context.Context, execCtx *ExecutionContext, bot *music.MusicBot, args map[string]interface{}) *ToolResult {
	...
	if bot.VoiceConn == nil || bot.VoiceConn.ChannelID != channelID {
		...
		if bot.VoiceConn != nil && bot.VoiceConn.ChannelID != channelID {
			log.Printf("Disconnecting from old voice channel: %s", bot.VoiceConn.ChannelID)
